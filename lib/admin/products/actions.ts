...
import {
  normalizeVariantForDb,
  removeProductImageObjectsByUrls,
  uploadProductImage,
} from "@/lib/admin/products/productImages";
...
type ProductImageListRow = { id: string; image_url: string; sort_order: number };
...
export async function updateProduct(formData: FormData) {
  ...
  if (removeIds.length) {
    ...
    await removeProductImageObjectsByUrls(
      rowsToRemove.map((row) => (row.image_url ?? "").trim()).filter(Boolean),
    );
  }
  ...
  const requestedPrimaryId = String(formData.get("primary_image_id") ?? "").trim();
  ...
  const requestedPrimary =
    requestedPrimaryId
      ? images.find((img) => img.id === requestedPrimaryId)
      : undefined;

  const primaryCandidate = requestedPrimary ?? images[0] ?? null;
  
  if (primaryCandidate) {
    await supabaseServer.from("product_images").update({ is_primary: false }).eq("product_id", id);
    await supabaseServer.from("product_images").update({ is_primary: true }).eq("id", primaryCandidate.id);
    await supabaseServer.from("products").update({ image_url: primaryCandidate.image_url }).eq("id", id);
  } else {
    await supabaseServer.from("products").update({ image_url: null }).eq("id", id);
  }
